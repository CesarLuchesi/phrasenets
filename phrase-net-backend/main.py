...
def validate_linking_params(
    linking_type: str = Form("orthographic", description="Linking method: 'orthographic' or 'syntactic'."),
    # CORRECTION: Using r"" to define the regex description string
    pattern: Optional[str] = Form(None, description=r"Regex Pattern (required for Orthographic Linking). Ex: r'(\w+)\s+(and)\s+(\w+)'"),
):
    ...
    if linking_type == 'syntactic' and pattern:
        print("Warning: The pattern will be ignored, as Syntactic Linking uses the dependency parser.")
...
async def analyze_text(
    ...
    # Injects and validates linking parameters
    ...
    try:
        ...
        # Catches Stanza/NLP not loaded error (RuntimeError from phrase_net_core.py)
        raise HTTPException(status_code=500, detail=f"NLP dependency error: {e}")
    except Exception as e:
        # Catches other processing errors
