    ...
    <div className="graph-visualization">
      <div className="graph-header">
        {data && (
          ...
          >
            {showTextPanel ? "🔼 Hide Text" : "📄 See text"}
          </button>
        )}
      </div>
      ...
      {!data && !loading && !error && (
        <div className="empty-state">
          <p> Paste text or upload a file to get started</p>
        </div>
      )}
      ...
      {data && !loading && (
        <>
          ...
          <div className="graph-stats">
            <div className="stat">
              <strong>Nodes:</strong> {data.node_count || 0}
            </div>
            <div className="stat">
              <strong>Edges:</strong> {data.edge_count || 0}
            </div>
          </div>
            ...
            <div className="text-panel">
              ...
              <div className="text-content">
                {loadingText ? (
                  ...
                  <p className="no-text">Text not available </p>
                )}
              </div>
            </div>
          )}
          ...
          {hoveredNode && (
            <div className="tooltip">
              ...
              <div>frequency: {hoveredNode.frequency || "N/A"}</div>
              <div>In-degree: {hoveredNode.inDegree || 0}</div>
              <div>Out-degree: {hoveredNode.outDegree || 0}</div>
              <div>Reason: {(hoveredNode.ratio || 0).toFixed(2)}</div>
            </div>
          )}

          {hoveredEdge && (
            <div className="tooltip edge-tooltip">
              <strong>Weigth: {hoveredEdge.weight || 1}</strong>
            </div>
          )}
        </>
      )}
